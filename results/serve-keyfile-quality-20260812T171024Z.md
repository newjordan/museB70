# Muse serve smoke — 2026-08-12T17:10:38Z

## browser origin policy
untrusted origin rejected; localhost origin allowed

## authentication
unauthenticated chat HTTP 401; authenticated chat accepted

## turn 1 (short)
content: 'MUSE-OK'
reasoning (first 120ch): 'Reply with exactly: MUSE-OK\n\nWe need reply with exactly: MUSE-OK\n\nProbably just output MUSE-OK. Exactly. No extra whites'
prompt_n=64 prompt_ms=793 predicted_per_sec=28.6

## turn 2 (same conversation extended — cache reuse check: prompt_n should be small)
content: 'SECOND-OK'
prompt_n=21 prompt_ms=449 predicted_per_sec=33.0

## template sanity (server log lines)
0.12.675.768 I srv          init: chat template supports preserving reasoning, consider enabling it via --reasoning-preserve

## slots / ctx at load
0.12.672.487 I srv    load_model: initializing, n_slots = 1, n_ctx_slot = 131072, kv_unified = 'false'
